...
export async function POST(req: Request) {
  try {
    ...
    const normalizedSourceUrl = asTrimmedString(sourceUrl) ?? asTrimmedString(url);
    ...
    const normalizedContent = asTrimmedString(content) ?? asTrimmedString(text);
    ...
    const { data: item, error } = await db
      ...
      .insert({
        ...
        ...(asTrimmedString(userId) && { userId: asTrimmedString(userId) }),
      })
      .select('id')
      .single();
    ...
    const normalizedImageUrl = asTrimmedString(imageUrl) ?? asTrimmedString(image);
    const normalizedFavicon = asTrimmedString(favicon);
    const customDataPayload = {
      ...(asObject(customData) ?? {}),
      ...(asTrimmedString(text) ? { extractedText: asTrimmedString(text) } : {}),
    };
