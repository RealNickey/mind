...
(() => {
  ...
  var MAX_SELECTION_CONTEXT_LENGTH = 800;
  var MAX_SELECTION_HTML_LENGTH = 4e3;
  ...
  function extractMetadata() {
    ...
    const title = document.querySelector('meta[property="og:title"]')?.getAttribute("content") || document.querySelector('meta[name="twitter:title"]')?.getAttribute("content") || article?.title || document.title || "";
    const description = document.querySelector('meta[property="og:description"]')?.getAttribute("content") || document.querySelector('meta[name="description"]')?.getAttribute("content") || article?.excerpt || "";
    const image = document.querySelector('meta[property="og:image"]')?.getAttribute("content") || document.querySelector('meta[name="twitter:image"]')?.getAttribute("content") || document.querySelector('meta[name="twitter:image:src"]')?.getAttribute("content") || "";
    const url = document.querySelector('link[rel="canonical"]')?.getAttribute("href") || document.querySelector('meta[property="og:url"]')?.getAttribute("content") || window.location.href;
  ...
  function extractSelection() {
    ...
    const html = selectionContainer.innerHTML ? clampText(selectionContainer.innerHTML, MAX_SELECTION_HTML_LENGTH) : void 0;
    let context;
    const contextNode = range.commonAncestorContainer;
    const contextElement = contextNode.nodeType === Node.ELEMENT_NODE ? contextNode : contextNode.parentElement;
  ...
  var QUICK_NOTE_OVERLAY_ID = "mymind-quick-note-overlay";
  var IMAGE_SAVE_BUTTON_ID = "mymind-image-save-button";
  function removeQuickNoteOverlay() {
    const overlay = document.getElementById(QUICK_NOTE_OVERLAY_ID);
    if (!overlay) {
      return;
    }
  ...
  function openQuickNoteOverlay(prefillText) {
    const existing = document.getElementById(QUICK_NOTE_OVERLAY_ID);
    if (existing) {
      const textarea2 = existing.querySelector("textarea");
      textarea2?.focus();
      return;
    }
    ...
    overlay.style.cssText = "position:fixed;inset:0;z-index:2147483647;display:flex;align-items:flex-end;justify-content:center;pointer-events:auto;";
    const backdrop = document.createElement("div");
    backdrop.style.cssText = "position:absolute;inset:0;background:rgba(15,23,42,0.18);";
    const card = document.createElement("div");
    card.style.cssText = 'position:relative;z-index:1;width:min(560px,92vw);margin:24px;padding:16px 16px 14px;background:#ffffff;border-radius:16px;box-shadow:0 24px 60px rgba(15,23,42,0.25);font-family:ui-sans-serif,system-ui,-apple-system,"Segoe UI",sans-serif;';
    ...
    textarea.style.cssText = "width:100%;min-height:110px;resize:vertical;border:1px solid #e2e8f0;border-radius:12px;padding:10px 12px;font-size:13px;line-height:1.5;color:#0f172a;outline:none;";
    ...
    cancelButton.style.cssText = "border:none;background:#e2e8f0;color:#0f172a;border-radius:10px;padding:8px 12px;font-size:12px;font-weight:600;cursor:pointer;";
    cancelButton.addEventListener("click", removeQuickNoteOverlay);
    const saveButton = document.createElement("button");
    saveButton.type = "button";
    saveButton.textContent = "Save to MyMind";
    saveButton.style.cssText = "border:none;background:#0f172a;color:#f8fafc;border-radius:10px;padding:8px 14px;font-size:12px;font-weight:600;cursor:pointer;";
  ...
  function setupImageHoverSave() {
    ...
    button.style.cssText = "position:absolute;z-index:2147483646;display:none;align-items:center;gap:6px;padding:6px 10px;border-radius:999px;background:rgba(15,23,42,0.92);color:#f8fafc;border:none;font-size:11px;font-weight:600;cursor:pointer;box-shadow:0 8px 24px rgba(15,23,42,0.25);";
  ...
  chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
    ...
    if (request?.action === "open-quick-note") {
      const prefillText = typeof request.prefillText === "string" ? request.prefillText : void 0;
